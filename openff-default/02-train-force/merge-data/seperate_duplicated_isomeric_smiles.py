...
def _duplicate_smiles(datasets):
    ...
    #duplicate = df[df['ISOMERIC_SMILES'].duplicated(keep='first')]
...
def run():
    ...
    for dataset in datasets:
        ...
        for p in paths_to_mydata:
            g = esp.Graph.load(p)
            #s = g.mol.to_smiles(isomeric=False, explicit_hydrogens=False)
